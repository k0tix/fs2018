    ...
    render() {
        const CountryNames = ({ countries, filter }) => {
            ...
            if(filteredCountries.length === 1) {
                ...
                return (
                    <div>
                        {filteredCountries.map(country => <div onClick={() => this.setState({filter: country.name})} key={country.name}> {country.name} </div>)}
                    </div>
                )
            }
        }
        ...
        return (
            <div>
                find countries: <input onChange={this.handleFilterChange} value={this.state.filter} />
